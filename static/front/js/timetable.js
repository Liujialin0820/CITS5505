window.addEventListener('load', function () {
  // Automatically scroll to 9:00 AM in the calendar view
  const body = document.querySelector('.calendar-body');
  if (body) {
    body.scrollTop = 540; // 60px * 9 hours
  }

  const courses = window.coursesData; // Data source: array of course objects with timeslots
  const grid = document.querySelector('.calendar-grid'); // Calendar visual grid

  const cellHeight = 60;          // Height per hour (in pixels)
  const timeColumnWidth = 80;     // Width of fixed time column on the left
  const totalGridWidth = grid.offsetWidth;

  const dayColumnWidth = Math.floor((totalGridWidth - timeColumnWidth) / 7); // Width for each day
  const lastColumnWidth = totalGridWidth - timeColumnWidth - dayColumnWidth * 6; // Width for Sunday (may adjust due to rounding)
  ...
  const courseColorMap = {}; // Map course name -> color
  let colorIndex = 0;

  // Render colored blocks on the calendar grid
  ...
  // Adjust block sizes on resize (if layout changes)
  function resizeClassBlocks() {
    ...
    document.querySelectorAll('.class-block').forEach(block => {
      const day = parseInt(block.dataset.day); // Expected to be set as dataset attribute
